...
# Fonction mettant à jour l'heure qu'il est dans notre simulation


def MiseAJourHeure(Distance):
    global Heure
    global Jour
    global HeureSimulation
    # Simulation du temps mis entre 2 villes
    HeureSimulation = Heure
    HeureSimulation += round(Distance/110)
...
for i in range(NmbIterations):
    # Nous supposons qu'il commence son trajet à 8h du au jour 0
    Jour = 0
    Heure = 8
    HeureSimulation = Heure
    ...
    for j in range(len(TrajetVilles)):
        ...
        MiseAJourHeure(Distance_Villes(VilleDepart, VilleEtape))
...
print(FinalMessage)
print(f"Le trajet à pris {Jour} jours et il arrivera à {Heure}h")
